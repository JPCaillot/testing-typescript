...
describe('Server test suite', () => {
    ...
    it('should handle register requests', async () => {
        ...
        expect(RegisterHandler).toBeCalledWith(requestMock, responseMock, expect.any(Authorizer)); // last one to check the correct type of argument instead of argument itself - small compromise
    });
    ...
    it('should handle errors in serving requests', async () => {
        ...
        expect(responseMock.writeHead).toBeCalledWith(
            HTTP_CODES.INTERNAL_SERVER_ERROR, JSON.stringify(`Internal server error: Some error`)
        );
    });
